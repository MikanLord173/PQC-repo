...
def gen_packet(obu_id):
    ...
    ecc_priv = serialization.load_pem_private_key(ecc_priv_bytes, password=None)
    
    with open(f"OBU/keys/{obu_id}_pqc_priv.key", "rb") as f:
        pqc_priv_bytes = f.read()
    ...
    with oqs.Signature(sig_name) as signer:
        ...
        # 將讀出來的 bytes 導入引擎
        signer.import_secret_key(pqc_priv_bytes)
    ...
    packet = msg_len + message + cert + ecc_sig + pqc_sig    # 組合成完整封包
